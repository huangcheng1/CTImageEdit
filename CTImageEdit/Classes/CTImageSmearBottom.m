...
- (void)changeState{
    if ([self.delegate respondsToSelector:@selector(hasLastMosaicOperation)]) {
        if ([self.delegate hasLastMosaicOperation]) {
            [self.lastBtn setImage:[[UIImage imageWithName:@"sccphoto_smear_last"]  tintImageWithColor:CTImageEditRGBColor(0xffffff, 1)] forState:UIControlStateNormal];
        }else{
            [self.lastBtn setImage:[[UIImage imageWithName:@"sccphoto_smear_last"]   tintImageWithColor:CTImageEditRGBColor(0x999999, 1)] forState:UIControlStateNormal];
        }
    }
    
    if ([self.delegate respondsToSelector:@selector(hasNextMosaicOperation)]) {
        if ([self.delegate hasNextMosaicOperation]) {
            [self.nextBtn setImage:[[UIImage imageWithName:@"sccphoto_smear_next"]  tintImageWithColor:CTImageEditRGBColor(0xffffff, 1)]  forState:UIControlStateNormal];
        }else{
            [self.nextBtn setImage:[[UIImage imageWithName:@"sccphoto_smear_next"]tintImageWithColor:CTImageEditRGBColor(0x999999, 1)]  forState:UIControlStateNormal];
        }
    }
}
...
- (UIButton *)commitBtn{
    if (!_commitBtn) {
        ...
        _commitBtn.titleLabel.font = [UIFont systemFontOfSize:14.0];
        [_commitBtn setBackgroundImage:[UIImage createImageWithColor:CTImageEditRGBColor(0xffffff, 1)] forState:UIControlStateNormal];
